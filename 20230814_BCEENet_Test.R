...
server <- function(input, output) {
	...
	if(!is.null(input$file1$datapath)){
		...
		PCA.dataframe <- fasta.file.subset(input$file1$datapath,matched.ID.list) %>%
		  make.pca.data.frame()
		
		PCA.results <- run.pca.analysis(PCA.dataframe,matched.ID.list,fasta.file.subset(input$file1$datapath,matched.ID.list))

		graph.labels <- PC.Labels(PCA.dataframe,matched.ID.list,fasta.file.subset(input$file1$datapath,matched.ID.list))
