...
get.potential.voucher.numbers<-function(file.path.fasta){
  ...
  for(i in 1:nrow(annotation.list)){
    ...
    # the annotations are sometimes separated by spaces and sometimes by under scores. this section and the following if else statment strips the voucher
    # numbers depending on the separator
...
match.vernet.to.fasta<-function(input.file){
  ...
  for (i in 1:nrow(matching.id.data.frame)) {
    ...
    else if(matching.id.data.frame$Genus[i]=="Cyanocitta" & str_contains(matching.id.data.frame$annotation[i],"Cyanocitta")){
      matching.id.data.frame$matching[i] <- 1
    }
